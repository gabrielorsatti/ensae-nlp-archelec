...
\textbf{Preprocessing.} Lowercasing, punctuation/digit removal, tokenization, three-layer stopword filtering (function words, domain terms, OCR watermarks), and lemmatisation via spaCy. Lemmatisation is preferred over stemming to produce valid dictionary forms rather than truncated word fragments, which is crucial for human interpretability in downstream analysis. Short documents ($<$100 characters) and German-language manifestos from Alsace-Moselle are excluded.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\linewidth]{exploration_base.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\linewidth]{heatmap_pcs_partis.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.65\linewidth]{optimal_k_clean.png}
\caption{$C_v$ coherence vs.\ number of topics. LDA peaks at $k{=}11$ with balanced distribution (no topic $>$24\%). NMF peaks at $k{=}5$ but concentrates 84.5\% of documents in 1 topic, rendering it analytically useless.}
\label{fig:coherence}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{lda_topic_words.png}
\caption{Top words per LDA topic (partisan vocabulary removed). Three clusters emerge: labour and class struggle (Topics 0, 5, 9), institutional governance (1, 3, 4), and issue-specific themes.}
\label{fig:topics}
\end{figure}

The 11 topics (Figure~\ref{fig:topics}) form three clusters. The \textbf{labour block} (Topics 0, 5, 9) spans employer--employee dynamics, militant Marxist vocabulary, and the 1981 Mitterrandist mobilisation. The \textbf{institutional block} (1, 3, 4) captures presidential-majority rhetoric, left-wing alliance language, and right-wing coalition discourse. \textbf{Issue topics} cover immigration (8), social protection (6), and ecology (2, 7, 10). 

\subsection{Temporal dynamics}

\begin{figure}[H]
\centering
\includegraphics[width=0.75\linewidth]{evolution_topics_v2.png}
\caption{Temporal evolution of topic shares across the five elections. Note the growing salience of immigration discourse (Topic 8) and ecology (Topic 7) from 1981 onward.}
\label{fig:temporal}
\end{figure}

The temporal evolution (Figure~\ref{fig:temporal}) captures genuine political dynamics. The immigration topic rises sharply after 1981, tracking the Front National's electoral breakthrough. The ecology cluster grows throughout the period, consistent with the institutionalisation of environmental politics. The class-struggle topics decline over time, mirroring the PCF's electoral erosion.
...
\begin{minipage}[t]{0.48\textwidth}
  ...
  \caption{Topic specialisation by political family. Niche families show strong over-representation.}
...
\begin{minipage}[t]{0.48\textwidth}
  ...
  \caption{Cosine similarity between family centroids in topic space. All values are close to 1.0.}
  \label{fig:cosine}
\end{minipage}
\end{figure}

Figure~\ref{fig:specialisation} quantifies thematic specialisation. Niche families (\textit{Gauche radicale}, \textit{Extrême droite}, \textit{Écologistes}) show strong over-representation in their signature topics. Mainstream families (\textit{Droite}, \textit{Centre}) have flatter profiles. Crucially, the cross-family similarity matrix (Figure~\ref{fig:cosine}) shows that all political families have cosine similarities close to 1.0 in the topic-distribution space. The differentiation happens not in \textit{what} they discuss but in \textit{how}, motivating Section~\ref{sec:framing}.
...
\caption{Normalised confusion matrix (logistic regression, 5-fold CV). Gauche radicale and Extrême droite achieve the highest diagonal values. Centre is most confused with Droite.}
\label{fig:confusion}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{lr_features_by_family.png}
\caption{Most discriminative TF-IDF features per family (logistic regression coefficients, partisan labels filtered out). Class struggle for the radical left, immigration for the far right, ecology for the Greens.}
\label{fig:features}
\end{figure}

After filtering party names, Figure~\ref{fig:features} exposes each family's ideological lexicon. These signatures align with topic-level specialisation but provide finer grain. Overall performance suggests that model expressiveness, not data quantity, is the limiting factor (detailed metrics and learning curves are provided in Appendix~\ref{app:metrics}).
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{fighting_words_employment.png}
\caption{Fighting words on the employment topic. The radical left frames unemployment through \textit{travailleurs} and \textit{patronat}; the far right through \textit{immigration} and \textit{insécurité}.}
\label{fig:fighting}
\end{figure}

Figure~\ref{fig:fighting} shows the employment topic. The radical left speaks of \textit{travailleurs}, \textit{patronat}, reflecting class conflict. The far right frames the same reality through \textit{immigration} and \textit{insécurité}. The moderate left uses a managerial register (\textit{emploi}, \textit{formation}). The right invokes \textit{entreprise} and \textit{charges}. Same policy domain, radically different rhetorical universes.

\subsection{Discourse vs.\ macroeconomic reality}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{macro_econ_extended.png}
\caption{Left: national inflation and unemployment (INSEE). Right: economic vocabulary frequency in manifestos by family. Discourse broadly tracks macroeconomic stress.}
\label{fig:macro}
\end{figure}
...
\caption{t-SNE projection of LDA topic vectors ($n{=}21{,}139$, perplexity${}=50$, cosine distance). Gauche radicale and Extrême droite occupy isolated regions; Droite and Centre overlap considerably.}
\label{fig:tsne}
\end{figure}
...
\begin{minipage}[t]{0.48\textwidth}
  ...
  \caption{F1 scores by political family. Gauche radicale and Extrême droite are the easiest to classify.}
  \label{fig:f1_scores}
\end{minipage}
\hfill
\begin{minipage}[t]{0.48\textwidth}
  ...
  \caption{Learning curve (5-fold CV). Performance plateaus at $\sim$60\% of training data.}
  \label{fig:learning_curve}
\end{minipage}
\end{figure}

Figure~\ref{fig:learning_curve} shows that the classifier's performance plateaus relatively early, indicating that model expressiveness (not data size) is the bottleneck. A more expressive approach, such as fine-tuning CamemBERT, would likely improve minority-class performance (Centre, Écologistes).
